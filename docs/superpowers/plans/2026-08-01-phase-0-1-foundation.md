...
- Produces:
  ```ts
  export interface NoteRef { id: string; path: string; title: string; mtimeMs: number }
...
describe('Vault', () => {
  ...
  it('emits external-change when another process writes', async () => {
    ...
    writeFileSync(p, '# two\n')
    await settle()
    expect(onEvent).toHaveBeenCalledWith({ type: 'external-change', path: p })
    stop(); await vault.close()
  })
})
```
...
Install the watcher: `pnpm add chokidar --filter @simplemark/core`
...
export class Vault {
  ...
  async write(path: string, text: string): Promise<void> {
    this.ledger.record(path, hashContent(text))
  ...
  watch(onEvent: (e: VaultEvent) => void): () => void {
    ...
    this.watcher.on('change', async (path) => {
      ...
      if (text === null) return
      if (this.ledger.isOwnWrite(path, hashContent(text))) return
      onEvent({ type: 'external-change', path })
    })
...
describe('paste into a note', () => {
  ...
  it('survives the full round trip: write, reopen, external edit', async () => {
    ...
    const updated = pasteIntoNote(await vault.read(path), payload, ctx)
    await vault.write(path, updated)
    await settle()

    expect(await vault.read(path)).toBe(updated)
    expect(onEvent).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'external-change' }))

    writeFileSync(path, updated + '\nadded elsewhere\n')
    await settle()
    expect(onEvent).toHaveBeenCalledWith({ type: 'external-change', path })
